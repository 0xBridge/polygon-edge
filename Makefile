...
build: check-go check-git
	...
	go build -o polygon-edge -ldflags="\
    	-X 'github.com/0xBridge/polygon-edge/versioning.Version=$(VERSION)' \
		-X 'github.com/0xBridge/polygon-edge/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github.com/0xBridge/polygon-edge/versioning.Branch=$(BRANCH)'\
		-X 'github.com/0xBridge/polygon-edge/versioning.BuildTime=$(TIME)'" \
	main.go
